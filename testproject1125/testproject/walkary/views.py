...
def save_steps(request):
    if request.method == 'POST':
        ...
        try:
            step_entry, created = WalkarySteps.objects.update_or_create(
                user=request.user,
                date=today,
                defaults={'steps': steps}
            )
            if created:
                message = "Steps entry created successfully"
            else:
                message = "Steps entry updated successfully"
            return JsonResponse({"message": message, "steps": step_entry.steps})
