...
function generateRoute() {
    ...
    const handleRouteGeneration = (waypointLocation) => {
        ...
        let waypoints = [];
        if (waypointLocation) {
            // 経由地点が指定されている場合はそれを使用
            waypoints = [{ location: waypointLocation, stopover: true }];
        } else {
            // 経由地点が指定されていない場合は距離に基づいてウェイポイントを生成
...
function updateWaypointAndDistanceState() {
    const startInput = document.getElementById('start').value.trim();  // トリムで空白除去
    const endInput = document.getElementById('end').value.trim();      // トリムで空白除去
    ...
    if (startInput && endInput) {
        if (startInput === endInput) {
            if (waypointInputValue) {
                // スタート地点とゴール地点が同じで、経由地点が入力された場合
                waypointInput.disabled = false;
                distanceInput.disabled = true;  // 距離は非アクティブ
            } else {
                // スタート地点とゴール地点が同じで、経由地点が入力されていない場合
                waypointInput.disabled = false;
                distanceInput.disabled = false;  // 距離はアクティブ
            }
        } else {
            // スタート地点とゴール地点が異なる場合、経由地点をアクティブ、距離を非アクティブ
            waypointInput.disabled = false;
            distanceInput.disabled = true;
        }
    } else {
        // いずれかが入力されていない場合、経由地点を非アクティブ、距離をアクティブ
